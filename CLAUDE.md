...
5. **Monthly payout** — Renda+ pays 240 monthly amortizations of `par / 240` each. In real terms, `par = V_c / Cotação_at_conversion`, so the real monthly payment uses the standard annuity PMT formula at the bond's invested-weighted real spread: `real_monthly = V_c × i / (1 - (1+i)^-240)` where `i = (1+spread)^(1/12) - 1`. This correctly accounts for the unamortized principal continuing to earn the spread throughout the 20-year payout. Total real cashflow over 240 months ≈ par (= V_c / Cotação ≈ V_c × 1.79 at spread 7%). IR = 15% on the gain portion (`gain_fraction = (total_real_cashflow − total_invested) / total_real_cashflow`). B3 fee assumed 0% (held to conversion, payment ≤ 6 minimum wages).
